...
function registerPredictHandlers(bot) {
  bot.command('predict', async (ctx) => {
    if (!ctx.from) return;
    const preferenceText = ctx.match?.trim();
  ...
  bot.callbackQuery(/^confirm_pick:(\d+):([A-Z0-9_]+):([A-Z]+)$/, async (ctx) => {
    ...
    await ctx.reply(
      `✅ Locked in: ${fixture.home_team} vs ${fixture.away_team} — ${selection} (${DEFAULT_STAKE} pts staked). ` +
      `I'll let you know the result after kickoff.`
    );
  });
  ...
  bot.command('mypicks', async (ctx) => {
    ...
    if (rows.length === 0) {
      return ctx.reply("You haven't made any picks yet. Try /predict to get started.");
    }
...
module.exports = { registerPredictHandlers };
